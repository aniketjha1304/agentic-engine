...
async def execute_workflow(
    ...
    if workflow.status != "active":
        raise ValueError(
            f"Workflow '{workflow_name}' is not active (status: {workflow.status})"
        )
    ...
    if workflow.input_parameters:
        try:
            ...
            error_msg = (
                f"Input validation failed for workflow '{workflow_name}': {e.message}"
            )
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(workflow.endpoint, json=input_data)
