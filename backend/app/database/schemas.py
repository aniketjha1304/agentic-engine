...
class Chat(BaseModel):
    ...
    agent_name: Optional[str] = Field(None, description="Name of the agent associated with this chat")
...
class Workflow(BaseModel):
    ...
    status: str = Field(default="inactive", description="Status of the workflow (active/inactive)")
    endpoint: Optional[str] = Field(None, description="API endpoint for the workflow")
    attributes: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional workflow attributes")
    description: Optional[str] = Field(None, description="Description of what the workflow does")
    input_parameters: Optional[Dict[str, Any]] = Field(None, description="JSON schema for workflow input parameters")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
...
class Agent(BaseModel):
    ...
    workflow_names: List[str] = Field(default_factory=list, description="List of workflow names this agent can use")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
